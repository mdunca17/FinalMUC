...
public class Diary extends Activity implements OnClickListener, OnKeyListener,OnItemSelectedListener, OnItemLongClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// sets up the array adapter with a simple list layout
	...
	private void addEntry(String item){
		...
		DateFormat dateFormat = new SimpleDateFormat("EEEE/dd/MMMM HH:mm");
 	    ...
 	    // gets a instance of calender to display date and time 
        Calendar cal = Calendar.getInstance();
		// creates entries
	if (item.length()>0){
		// adds string from edit text, venue string and the date/time to entrys array adapter
	...
	public void onClick(View V) {
		...
		//if button add entries is presses then get text from edit text
	if(V == this.AddEntry){
			this.addEntry(this.Entry.getText().toString());
			
		}
	// Start main activity when back button is pressed
	if (V == Back){
	...
	// if the key event occurs add entry to list view 
	...
	public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2,
		...
		// checks what spinner item is selected and sets venue string to its name to display in edit text
